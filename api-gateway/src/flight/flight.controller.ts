...
import { ControlMSG, FlightMSG, PassengerMSG } from 'src/common/constants';
...
export class FlightController {
  ...
  private _clientProxyControl = this.clienProxy.clientProxyControls();
  ...
  @Post(':flightId/avion/:controlId')
  async addControl(
    @Param('flightId') flightId: string,
    @Param('controlId') controlId: string,
  ) {
    const control = await this._clientProxyControl
      .send(ControlMSG.FIND_ONE, controlId)
      .toPromise();

    if (!control)
      throw new HttpException('Avion not found', HttpStatus.NOT_FOUND);
    return this._clientProxyFlights.send(FlightMSG.ADD_CONTROL, {
      flightId,
      controlId,
    });
  }
